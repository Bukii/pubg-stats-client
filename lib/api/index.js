...
exports.tele = async function (url, token) {
  let result = await API(url, token)
  return new Promise((resolve, reject) => {
    return resolve(result)
  })
}
...
exports.player = {
  ...
  lifetimeStatsInfo: async function (region, id, token) {
    let result = await API(`https://api.playbattlegrounds.com/shards/${region}/players/${id}/seasons/lifetime`, token)
    return new Promise((resolve, reject) => {
      return resolve(result)
    })
  },
  seasonStatsInfo: async function(region, id , seasonId , token) {
    let result = await API(`https://api.playbattlegrounds.com/shards/${region}/players/${id}/seasons/${seasonId}`, token)
    return new Promise((resolve, reject) => {
      return resolve(result)
    })
  }
